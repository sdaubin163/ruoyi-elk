...
public class SysUserController extends BaseController
{
    ...
    public TableDataInfo list(SysUser user)
    {
        ...
        log.info("开始模拟测试服务间调用111");
        TestTraceIdVo traceIdVo = new TestTraceIdVo();
        traceIdVo.setTest_traceid(UUID.randomUUID().toString().replaceAll("-",""));
        remoteFileService.test_traceid(traceIdVo);
        log.info("完成模拟测试服务间调用111");


        FpsMonVo fpsMonVo = new FpsMonVo();
        fpsMonVo.setSysId("ruoyi");
        fpsMonVo.setSysName("若依管理台");
        fpsMonVo.setTransId("list");
        fpsMonVo.setTransName("获取用户列表");
        fpsMonVo.setTransStatus("0");
        log.info("插入监控信息， {}", fpsMonVo.toString());
